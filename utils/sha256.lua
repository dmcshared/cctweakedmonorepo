...
local function digestChunk(chunk, hash)
    for i = 17, 64 do
        local s0 = bit.bxor(bit.brshift(chunk[i - 15], 3),
            bit.bxor(rrotate(chunk[i - 15], 7), rrotate(chunk[i - 15], 18)))
        local s1 = bit.bxor(bit.brshift(chunk[i - 2], 10),
            bit.bxor(rrotate(chunk[i - 2], 17), rrotate(chunk[i - 2], 19)))
